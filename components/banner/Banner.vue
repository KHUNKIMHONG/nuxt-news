<template>
  ...
  >
    ...
    <div class="container mx-auto">
      ...
      <div class="absolute bg-gradient-to-r from-blue-950 via-blue-900 to-blue-950 w-96 h-96 md:w-[15rem] md:h-[15rem] rounded-full -right-20 -top-1"></div>

      <div
        class="slide flex w-full h-full mr-auto transition-opacity duration-700 ease-in-out"
      ...
      >
        ...
        <div
          class="flex flex-col w-full md:w-1/2 items-start justify-center text-left pl-40 space-y-4 z-10"
        >
          <!-- Slide Title -->
          <h3
            class="text-3xl md:text-4xl font-bold text-white leading-tight md:leading-snug tracking-wide max-w-xl"
          >
            {{ slide.title }}
          </h3>
          ...
          <!-- Buttons -->
          <div
            class="flex flex-col md:flex-row items-start justify-start space-y-4 md:space-y-0 md:space-x-4 mt-4"
          >
            <!-- Watch Video Button -->
            <button
              v-if="!isVideoVisible"
        ...
        <!-- Image Display -->
        <div
          class="w-full lg:w-1/2 h-full flex items-center justify-center p-6 lg:p-10 z-10"
        >
          <img
            :src="slide.image"
            alt="Slide Image"
...
<style scoped>
.logo-image {
  height: 3rem; /* Image size */
  opacity: 0; /* Start invisible */
  transform: translateY(50px); /* Start off-screen slightly (lower) */
  animation: fadeInSlideUp 1.5s ease-out forwards; /* Updated animation */
  transition: transform 0.3s, box-shadow 0.3s;
}

.logo-image:hover {
  box-shadow: 0px 8px 20px rgba(0, 0, 0, 0.2); /* Soft shadow on hover */
