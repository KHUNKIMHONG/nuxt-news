<template>
  <client-only>
    <nav
      class="sticky top-0 w-full bg-gradient-to-r from-emerald-700 via-teal-700 to-emerald-700 z-20 transition duration-300 flex justify-between items-center"
      x-data="{ menuOpen: false }">
      <!-- Logo Section -->
      <NuxtLink to="/" class="flex items-center mx-4 transition duration-300 hover:no-underline">
        <!-- <img src="/tonle-rm.png" alt="Logo" class="h-16 transition duration-300 hover:shadow-lg hover:blur-sm"
          loading="lazy" /> -->
        <div class="text-center p-2">
          <h3
            class="font-khmer font-bold text-white dark:text-white transition duration-300 hover:text-yellow-400 dark:hover:text-yellow-300">
            <strong
              class="text-yellow-500 transition duration-300 hover:text-red-500 dark:hover:text-red-400">ឃុន</strong>
            គឹមហុង
          </h3>
          <h5
            class="text-white text-lg font-semibold tracking-wide transition duration-300 hover:text-yellow-400 dark:hover:text-yellow-300">
            KHUN KIMHONG
          </h5>
        </div>
      </NuxtLink>
      ...
      <!-- Navbar Links -->
      <div :class="{
        'flex flex-col bg-teal-600 p-4 rounded-md space-y-4 absolute mt-2 top-20 left-0 right-0': menuOpen,
        hidden: !menuOpen,
      }"
        ...
        <ul class="flex flex-col md:flex-row md:space-x-3 w-full md:w-auto">
          <li v-for="link in LinkNavbar" :key="link.name_en" class="relative w-full md:w-auto text-center md:text-left"
            ...
            <NuxtLink :to="link.path" :class="getLinkClass(link.path)"
              class="flex px-4 py-2 w-full font-medium md:w-auto text-white dark:text-white hover:text-yellow-500 rounded-md transition items-center justify-between">
            ...
            <ul v-if="link.children" v-show="dropdownOpen === link.name_en"
              class="bg-white dark:bg-gray-800 text-black dark:text-white text-left px-4 py-4 rounded-md shadow-lg space-y-2 mt-2 
              transition-all duration-200 ease-in-out lg:absolute lg:top-full lg:left-0 lg:z-10 lg:w-48 lg:p-3 lg:rounded-md">
              ...
              hover:bg-gradient-to-r hover:from-blue-100 hover:to-emerald-300 dark:hover:from-gray-700 dark:hover:to-gray-600
            hover:text-emerald-900 dark:hover:text-yellow-400">
      ...
      <div class="md:hidden mr-6">
        <button @click="menuOpen = !menuOpen" aria-label="Toggle Menu"
          class="flex items-center justify-center p-2 transition-transform duration-300 ease-in-out rounded-md border border-gray-300 text-white hover:bg-yellow-500 dark:border-gray-600 dark:text-gray-300 dark:hover:bg-gray-700">
      ...
      <div class="hidden md:flex items-center space-x-4 mr-6">
        ...
        <div class="relative">
          <a href="/path-to-your-cv.pdf" download
            class="flex items-center px-4 py-2 border border-gray-300 focus:outline-none ring-2 ring-emerald-500 dark:border-gray-600 rounded-md bg-gray-50 dark:bg-gray-700 text-gray-800 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600 transition duration-300">
            <Icon name="mdi:download" class="h-5 w-5 text-gray-500 dark:text-gray-300 mr-2" />
            Download CV
          </a>
        </div>
        ...
        <button @click="switchTheme" @mouseenter="showTooltip = true" @mouseleave="showTooltip = false"
          class="relative flex items-center justify-center p-2 rounded-md border border-gray-300 ring-2 ring-emerald-500 dark:border-gray-600 transition duration-300 ease-in-out focus:outline-none"
        ...
        <div class="relative inline-block text-left">
          ...
          <button v-if="currentLanguage" @click="languageOpen = !languageOpen"
            class="text-xs text-white dark:text-gray-300 px-4 py-2 rounded-md border border-gray-300 ring-2 ring-emerald-500 dark:border-gray-600 hover:bg-gray-200 hover:text-yellow-500 dark:hover:bg-gray-700 dark:hover:text-yellow-500 focus:outline-none flex items-center">
          ...
          <div v-if="languageOpen" v-click-outside="() => languageOpen = false"
            class="absolute right-0 z-50 w-40 bg-white dark:bg-gray-800 shadow-lg mt-1 rounded-md overflow-hidden">
            <ul class="py-2 px-2">
...
// Updated getLinkChildren to support nested routes
const getLinkChildren = (childPath) => {
  const isActive = route.path.startsWith(childPath);
  return isActive
    ? "text-white p-2 bg-emerald-500 dark:bg-emerald-600  font-semibold transition duration-200 ease-in-out"
